...
def count_coins(change):
    ...
    def helper(change, max_coin):
        if not max_coin:
            ...
            return 1
        return helper(change, next_smaller_coin(max_coin)) + helper(change - max_coin, max_coin)
...
def beaver(f):
    "*** YOUR CODE HERE ***"
    # triple = lambda g: lambda f: g(g(g(f)))
    (lambda g: g(g(g(lambda _: f()))))((lambda g: lambda f: g(g(g(f))))(lambda g: lambda f: g(g(g(f)))))(f)
    # 3 ** 3 ** 3 
